...
static int Usage()
{
    Console.WriteLine("T48Probe commands:");
    ...
    Console.WriteLine("  erase-chip [log-file]");
...
try
{
    ...
    if (command == "read-flash")
    {
        ...
        using var device = T48UsbDevice.OpenFirst(fileLogger);
        var spi25 = new T48Spi25Client(device);
        var data = spi25.ReadFlash(offset, length);
    ...
    if (command == "blank-check")
    {
        ...
        using var device = T48UsbDevice.OpenFirst(fileLogger);
        var spi25 = new T48Spi25Client(device);
        var result = spi25.BlankCheck(offset, length);
        if (result.IsBlank)
        {
